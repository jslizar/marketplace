...
| `copy-post` | Example-driven drafting: reverse-engineer any LinkedIn post into a style spec, then draft a style-matched, funnel-classified, ICP-personalized post. Use when you have a post to imitate; use content-engine when you want a format from the library. |
...
| `content-interview` | Client content-intake pipeline: run the interview → build a deep emotional ICP avatar → mine the transcript into 20+ tagged post concepts → draft to hook-lever writing standards. Feeds `content-engine`. No dependencies. |
