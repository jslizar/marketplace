...
2. **Library** — asks which format library to pull from: **Virio** (corpus-mined, auto-refreshed) or **Millie's list** (curated from `Viral_Content_Bank.xlsx`). Always asks unless you name one.
...
```
content-library/
├── virio/          tofu (15) · mofu (15) · bofu (15) · abm (15)   ← auto-refreshed
└── millies/        tofu (13) · mofu (9)  · bofu (0)  · abm (5)    ← curated, manual updates
```

Millie's BOFU is empty by design — her Bottom of Funnel tab is all ABM-style posts; those live in `millies/abm/`. Each lane folder has a `config.md` (directive + admission test + refresh policy) and an `index.md`. Millie's indexes list "unread candidates" — bank posts not yet deconstructed; say "deconstruct this post" to spec one.
