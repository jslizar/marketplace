# Soft-skill framework

- **Archetype**: A values claim ("X = real strength") backed by a numbered why + a named how
- **Funnel stage**: TOFU — earns reach with the ICP; client-agnostic on the surface.
- **Stage mechanic**: Turns a feel-good belief the ICP already holds into an argued, structured framework they can repost as their own leadership philosophy. Riffing off another creator's take ("Saw {name} drop his POV and had to share mine") borrows an existing audience.
- **Platform**: LinkedIn
...
- **Skeleton** (block sequence):
  ...
  2. Credit line to the creator being riffed on
  3. The reframe sentence (what the world gets wrong about this trait)
  4. "Why {trait} = {strength}": numbered list of 3, each with one explaining line
  5. "How to {practice it}": 3 named moves with one-line instructions
  6. "The ripple effect": the payoff paragraph
- **Rhythm & formatting**: 180–250 words. Numbered + dashed structure. Bold section labels. One emoji in the hook, none after.
- **Why it works**: Structure launders sentiment into insight — the same belief as a plain statement gets scrolled past; as a framework it gets saved. The riff credit invites the original creator's audience in.
