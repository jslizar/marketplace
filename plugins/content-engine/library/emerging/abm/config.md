...
- **Job**: Glorify or target a specific named account, person, team, or campaign so the target engages.
- **What "emerging" means**: every entry here is a spec-lite deconstructed from a post that
  ...
  proven over time. Same lane job as Virio abm; the difference is recency, not intent.
...
- Contents = current week only. Each Saturday the outgoing batch rolls into `virio/abm/` as
