# Emerging — bofu config

## Lane directive

- **Job**: Convert existing intent — de-risk the decision with verifiable proof.
- **What "emerging" means**: every entry here is a spec-lite deconstructed from a post that
  ...
  proven over time. Same lane job as Virio bofu; the difference is recency, not intent.
...
- Contents = current week only. Each Saturday the outgoing batch rolls into `virio/bofu/` as
