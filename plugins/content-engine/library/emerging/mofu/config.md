# Emerging — mofu config

## Lane directive

- **Job**: Build trust — educate, show the method, soft proof.
- **What "emerging" means**: every entry here is a spec-lite deconstructed from a post that
  ...
  proven over time. Same lane job as Virio mofu; the difference is recency, not intent.
...
- Contents = current week only. Each Saturday the outgoing batch rolls into `virio/mofu/` as
