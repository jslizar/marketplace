...
Resolution: the pick is MULTI-SELECT — one, two, or all three. If the user named libraries ("use Millie's list", "from Virio", "emerging", "what's working this week", "virio and millie's", "all of them") → use exactly those. Otherwise ALWAYS ask — one multi-select question, three options with a one-line description of each, phrased so it's clear they can pick more than one ("pick one or more"). Never silently default.

When multiple libraries are selected, they form one pooled candidate set for the lane: formats compete across libraries on fit, and every format is labeled with its source library wherever it appears (cards, tables, draft frontmatter). If one selected library has nothing for the lane (Millie's bofu, a thin Emerging week), say so in one line and continue with the rest — it only becomes a stop if NO selected library has specs for the lane.
...
Once the library selection and lane are set (Step 1 above + the lane from Step 0), verify each selected library's local lane against the source before reading formats from it — with multiple libraries selected, run the check once per selected library. This is what heals a stale `content-library/` left by an older version — VERSION equality alone is not trusted for the lane you're about to use.
...
4. **Angles** (angles): Cross the CHOSEN format(s) — not the whole library — against the client context, market research, and the lane directive from `content-library/<library>/<lane>/config.md`, where `<library>` is each chosen format's HOME library (with a pooled pick, a format is governed by its own library's lane config, not the other selected libraries'). Present the angles as a compact ranked TABLE in the chat (rank, subject, format it pairs with, why-now, payoff, score) — never as a widget or card form. Ask for the pick with a plain question. STOP — the user picks the angle(s).
