...
On first run (no `content-library/` in the working folder), seed BOTH libraries by copying `${CLAUDE_PLUGIN_ROOT}/library/` into `content-library/`, including `library/VERSION`. On every later run, compare `${CLAUDE_PLUGIN_ROOT}/library/VERSION` against `content-library/VERSION`: if the bundled version is newer, say a library update shipped with the plugin and offer a non-destructive merge — copy in new spec files, refresh unpinned examples in unmodified specs, update `content-library/VERSION`. Never overwrite pinned examples or specs the user has edited; when unsure whether a file was edited, show the diff and ask.

## Pipeline

1. **Formats** (scout): Ensure `content-library/<library>/<lane>/` exists and has specs. The seeded library is authoritative — Virio lanes are refreshed on the maintainer's machine and reach everyone else as plugin updates, so only flag staleness (per the lane config's freshness window) when this machine runs the scheduled refresh task; never tell a non-maintainer their refresh failed. Millie's lanes are curated — never flag staleness, never auto-refresh.
