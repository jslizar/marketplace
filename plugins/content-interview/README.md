...
- **interview-kit** — prep and run the client content interview. Say "prepare for a content interview with [name]" and it researches the FOC in Virio (strategy, tone, ICPs, banned topics) plus their recent LinkedIn posts, then builds a tailored question set: cutting what the record already answers, flagging the gaps to fill, and deriving questions from their real ICP personas. Also carries the funnel-stage question banks, capture guidance, and the cadence for interview #2 and beyond.
...
- **Virio connector recommended.** It powers interview prep: `content_publishers_list` resolves the FOC, `content_settings_get` reads their strategy/tone/ICPs (read-only — the skill never writes back), and `read_linkedin_uri` pulls what they've already posted. HubSpot/Attio add account context if connected.
